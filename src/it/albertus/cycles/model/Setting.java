...
public enum Setting { // The order matters.
	...
	OVERSPEED_GRACE_PERIOD, // 6-7: periodo di grazia su red mark (valore alto: il motore si rompe dopo piu' tempo. Per valori msB >=80 si rompe subito).
	GRIP, // 8-9: soglia di slittamento in sterzata (valore alto: slitta meno).
	UNKNOWN_1, // 10-11: ?
	BRAKING_SPEED, // 12-13: velocita' di frenata.
	UNKNOWN_2, // 14-15: ?
	SPIN_THRESHOLD, // 16-17: soglia di testacoda (valore basso: testacoda piu' probabile. Per valori msB >=80 testacoda sicuro).
	UNKNOWN_3, // 18-19: ?
	RPM_DOWNSHIFT; // 20-21: regime di scalata con cambio automatico (skill < 3).
	
	public String toString() {
		return WordUtils.uncapitalize( WordUtils.capitalizeFully( this.name(), new char[] {'_'} ).replace( "_", "" ) );
	}
